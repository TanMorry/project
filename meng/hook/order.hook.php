...
function cart_list() {
	...
	foreach ($cart_list as $k=>$v) {
		...
		if ($v['prorule_key']) {
			$sql = "select a.`product_id`, a.`product_name`, a.`product_logo`, a.`product_money`, a.`product_smoney`, b.`product_money` as `prorule_money`, a.`product_wlmoney`, a.`product_point`, b.`product_num` as `product_maxnum`, a.`product_rule`, b.`prorule_name`, a.`category_id` from `".dbpre."product` a, `".dbpre."prorule` b where a.`product_id` = b.`product_id` and b.`product_id` = '{$v['product_id']}' and b.`prorule_key` = '{$v['prorule_key']}'";
			...
			$product = $db->pe_select('product', array('product_id'=>$v['product_id']), "`product_id`, `product_name`, `product_logo`, `product_money`, `product_wlmoney`, `product_point`, `product_num` as `product_maxnum`, `product_rule`,`category_id`");
			//如果管理后台已经把购物者中这个商品的规格或者变更了
...
//开发票状态计算
function invo_stateshow($info, $type = 'html') {
	if ($info['order_invo'] == 0) {
		$text = '不开票';
		$html = "<span class=\"corg\">{$text}</span>";
	}
	elseif ($info['order_invo'] ==1) {
		$text = '开票成功';
		$html = "<a href=\"admin.php?mod=order&act=invo&id={$info['order_id']}\" target=\"_blank\" class=\"c999\"><span class=\"cgreen\">{$text}</span></a>";
	}
	elseif ($info['order_invo'] == 2) {
		$text = '开票失败';
		$html = "<span class=\"corg\">{$text}</span>";
	}
	return $type == 'html' ? $html : $text;
...
function order_callback_pay($order_id, $order_outid = '', $order_payway = '') {
	...
	if (order_table($order_id) == 'order_pay') {
		...
		if(info['order_state']=='wpay'&&info['order_pstate']==0){
			$user = $db->pe_select('user', array('user_id'=>$info['user_id']));
			$sql_addpoint['user_point']=intval($info['order_money'])+intval($user['user_point']);
			$sql_addpoint['user_point_all']=$info['order_money']+$user['user_point_all'];
			$db->pe_update('user', array('user_id'=>$info['user_id']),$sql_addpoint);
		}
