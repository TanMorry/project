	...
	default:
		...
		if (isset($_p_pesubmit)) {
			...
			if ($db->pe_update('user', array('user_id'=>$_s_user_id), pe_dbhold($sql_set))) {
				if ($_FILES['user_logo']['size']) {
					pe_lead('include/class/upload.class.php');
					$upload = new upload($_FILES['user_logo'], 'data/avatar/', array('filename'=>$_s_user_id));
					$db->pe_update('user', array('user_id'=>$_s_user_id), array('user_logo'=>$upload->filehost));
				}
				pe_success('修改成功！');
			}
			else {
