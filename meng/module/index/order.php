...
switch ($act) {
	...
	case 'add':
		...
		if (isset($_p_pesubmit)) {
			...
			$sql_order['order_invo'] = $_p_order_invo;
			...
			if ($db->pe_insert('order', pe_dbhold($sql_order))) {
				...
				//发票生成
                if($_p_invo==1){
                    if($_p_invo_type==0||$_p_invo_type==1){
                        $sql_invo['order_id']=$sql_order['order_id'];
                        $sql_invo['order_status']===0;
                        $sql_invo['invo_type']=$_p_invo_type;
                        $sql_invo['invo_name']=isset($_p_invo_name1)?$_p_invo_name1:$_p_invo_name2;
                        $sql_invo['invo_duty']=$_p_invo_duty;
                        $sql_invo['invo_addr']=$_p_invo_addr;
                        $sql_invo['invo_phone']=$_p_invo_phone;
                        $sql_invo['invo_bank']=$_p_invo_bank;
                        $sql_invo['invo_account']=$_p_invo_account;
                        foreach ($info_list as $v) {
                            $category_name =$db->pe_select('category',array('category_id'=>$v['category_id']),'category_name');
                            $sql_invo_content['category_name'] =$category_name['category_name'];
                            $sql_invo_content['product_money'] = $v['product_money'];
                            $sql_invo_content['product_num'] = $v['product_num'];
                            $invo_content[]=$sql_invo_content;
                        }
						$invo_content['order_quan_money'] =  is_null($sql_order['order_quan_money'])?0:$sql_order['order_quan_money'];
						$invo_content['order_point_money'] = is_null($sql_order['order_point_money'])?0:$sql_order['order_point_money'];
						$invo_content['order_wl_money'] = $sql_order['order_wl_money'];
                        $invo_content['order_money']= $money['order_money'];
                        $json_invo_content = json_encode($invo_content,JSON_UNESCAPED_UNICODE);
						$sql_invo['invo_content']=$json_invo_content;
                    }
                    if( $db->pe_insert('invoice', pe_dbhold($sql_invo,array('invo_content')))){
                        $db->pe_update('order', array('order_id'=>$order_id), array('order_invo'=> 1));
                    }else{
                        $db->pe_update('order', array('order_id'=>$order_id), array('order_invo'=> 2));
                    }
                }
