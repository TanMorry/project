...
switch ($act) {
    ...
    default:
        ...
        $field = "product_id,product_name,product_text,product_logo,user_name,product_linian";
        if($category_id){
//            $product_info =$db->pe_selectall('product',array('category_id'=>$category_id),'product_id,product_name,product_text,product_logo',array(10,$page));
            $sql = "select {$field} from ".dbpre."product a left join ".dbpre."user b on a.userid=b.user_id where category_id = {$category_id} and user_id!=''";
            if(!empty($brand_id)){
...
//            $product_info =$db->pe_selectall('product','','product_id,product_name,product_text,product_logo',array(10,$page));
            $sql = "select {$field} from ".dbpre."product a left join ".dbpre."user b on a.userid=b.user_id where user_id!=''";
            if(!empty($brand_id)){
