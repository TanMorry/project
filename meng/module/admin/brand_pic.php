...
//			$_p_info['brand_word'] = strtoupper(substr($pinyin->output($_p_info['brand_name']), 0, 1));
    ...
    //#####################@ 查看子图 @#####################//
    case 'detail':
        switch (trim($_GET['detail_act'])){
            case 'add':
                $brand_pic_detail_id = intval($_g_id);
                if (isset($_p_pesubmit)) {
                    pe_token_match();
//			pe_lead('include/class/pinyin.class.php');
//			$pinyin = new pinyin();
//			$_p_info['brand_word'] = strtoupper(substr($pinyin->output($_p_info['brand_name']), 0, 1));
                    $_p_pic_info['brand_pic_addtime'] = $_p_pic_info['brand_pic_edittime'] = time();
                    $_p_pic_info['brand_pic_pid'] = $brand_pic_detail_id;
                    $_p_pic_info['brand_pic_bid'] = $_p_pic_info['brand_pic_bid'];
                    $_p_pic_info['brand_pic_bname'] = $_p_pic_info['brand_pic_bname'];
//                    var_dump($_p_pic_info);die;
                    if ($brand_pic_id = $db->pe_insert('brand_pic', pe_dbhold($_p_pic_info))) {
                        if ($_FILES['brand_pic']['size']) {
                            pe_lead('include/class/upload.class.php');
//                    var_dump($_FILES);die;
                            $upload = new upload($_FILES['brand_pic'], 'data/attachment/brand/pic/'.$brand_pic_detail_id.'/', array('filename'=>$brand_pic_id));
                            $db->pe_update('brand_pic', array('brand_pic_id'=>$brand_pic_id), array('brand_pic_addr'=>$upload->filehost));
                        }
                        cache_write('brand_pic_detail');
                        pe_success('添加成功!', 'admin.php?mod=brand_pic&act=detail&id='.$brand_pic_detail_id);
                    }
                    else {
                        pe_error('添加失败!');
                    }
                }else{
                    $pic_info = $db->pe_select('brand_pic', array('brand_pic_id'=>$brand_pic_detail_id),'brand_pic_bname,brand_pic_bid');
                }
                $seo = pe_seo($menutitle='添加品牌子图图片', '', '', 'admin');
                include(pe_tpl('brand_pic_detail_add.html'));
            break;
            case 'edit':
                $brand_pic_detail_id = intval($_g_id);
                $brand_pic_detail_editid = intval($_g_editid);
                if (isset($_p_pesubmit)) {
                    pe_token_match();
                    if ($_FILES['brand_pic']['size']) {
                        pe_lead('include/class/upload.class.php');
                        $upload = new upload($_FILES['brand_pic'], 'data/attachment/brand/pic/'.$brand_pic_detail_id.'/', array('filename'=>$brand_pic_detail_editid));
                        $db->pe_update('brand_pic', array('brand_pic_id'=>$brand_pic_detail_id), array('brand_pic_addr'=>$upload->filehost));
                    }
//			pe_lead('include/class/pinyin.class.php');
//			$pinyin = new pinyin();
//			$_p_info['brand_word'] = strtoupper(substr($pinyin->output($_p_info['brand_name']), 0, 1));
                    ...
                    $_p_pic_info['brand_pic_edittime'] = time();
                    if ($db->pe_update('brand_pic', array('brand_pic_id'=>$brand_pic_detail_id), pe_dbhold($_p_pic_info))) {
                        cache_write('brand_pic_detail');
                        pe_success('修改成功!', 'admin.php?mod=brand_pic&act=detail&id='.$brand_pic_detail_editid);
                    }
                    else {
                        pe_error('修改失败...');
                    }
                }
                $pic_info = $db->pe_select('brand_pic', array('brand_pic_id'=>$brand_pic_detail_id));
                $seo = pe_seo($menutitle='修改子图图片', '', '', 'admin');
                include(pe_tpl('brand_pic_detail_add.html'));
            break;
            case 'del':
                pe_token_match();
            ...
            default :
                $brand_pic_detail_id = intval($_g_id);
                $info_list = $db->pe_selectall('brand_pic', array('brand_pic_pid'=>$brand_pic_detail_id,'order by'=>'brand_pic_id desc'), '*', array(30, $_g_page));
                $parent_pic = $db->pe_select('brand_pic', array('brand_pic_id'=>$brand_pic_detail_id));
                $tongji['all'] = $db->pe_num('brand_pic', array('brand_pic_pid'=>$brand_pic_detail_id));
                $seo = pe_seo($menutitle='品牌管理', '', '', 'admin');
                include(pe_tpl('brand_pic_detail_list.html'));
            break;
        }
    break;
	//#####################@ 品牌原图片列表 @#####################//
