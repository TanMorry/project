...
switch ($act) {
	...
	case 'email_test':
		...
		foreach (explode(',', $_g_user) as $k=>$v) {
			...
			$sql_set[$k]['noticelog_name'] = '九千萌电子商城系统测试邮件';
			$sql_set[$k]['noticelog_text'] = '尊敬的用户：您好，欢迎使用简好网络旗下软件 - 九千萌电子商城系统<br/><br/>简好网络官网：http://www.phpshe.com<br/><br/>邮件发送日期：'.pe_date(time());
	...
	case 'sms_test':
		...
		foreach (explode(',', $_g_user) as $k=>$v) {
			...
			$sql_set[$k]['noticelog_text'] = "【简好网络】尊敬的用户：您好，欢迎使用简好网络旗下软件 - 九千萌电子商城系统，官网：http://www.phpshe.com";
			$sql_set[$k]['noticelog_atime'] = time();			
		}
		if ($db->pe_insert('noticelog', $sql_set)) {
			pe_success('发送成功!');
