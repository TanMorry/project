...
switch ($act) {
	...
	case 'invo':
		$order_id = pe_dbhold($_g_id);
		$info = $db->pe_select('order', array('order_id'=>$order_id));
		$invo_info = $db->pe_select('invoice',array('order_id'=>$order_id));
		$invo_list = json_decode($invo_info['invo_content'],TRUE );
		$order_quan_money = $invo_list['order_quan_money'];
		$order_quan_money = $invo_list['order_point_money'];
		$order_quan_money = $invo_list['order_wl_money'];
		$order_quan_money = $invo_list['order_money'];
		unset($invo_list['order_quan_money']);
		unset($invo_list['order_point_money']);
		unset($invo_list['order_wl_money']);
		unset($invo_list['order_money']);
		//print_r($invo_list);
		//var_dump($invo_list);
		$seo = pe_seo($menutitle='发票信息', '', '', 'admin');
		include(pe_tpl('order_invoinfo.html'));
	break;
	//#####################@ 订单删除 @#####################//
	case 'del':
		pe_token_match();
