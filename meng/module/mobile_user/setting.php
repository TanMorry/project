...
switch($act) {
	...
	case 'pw':
		...
		if (isset($_p_pesubmit)) {
			...
			if (!$db->pe_num('user', array('user_id'=>$_s_user_id, 'user_pw'=>md5($_p_user_oldpw)))) {
				pe_jsonshow(array('result'=>false, 'show'=>'原密码错误'));
			}
	...
	default:
		...
		if (isset($_p_pesubmit)) {
			pe_token_match();
			if ($_p_user_phone && !pe_formcheck('phone', $_p_user_phone)) pe_jsonshow(array('result'=>false, 'show'=>'请填写正确的手机号'));
			if ($_p_user_email && !pe_formcheck('email', $_p_user_email)) pe_jsonshow(array('result'=>false, 'show'=>'请填写正确的邮箱'));
