...
switch($act) {
	...
	case 'close':
		...
		if (isset($_p_pesubmit)) {
			if ($info['order_state'] == 'close') pe_jsonshow(array('result'=>false, 'show'=>'订单已关闭，请勿重复操作'));
			if (in_array($info['order_state'], array('wget', 'success'))) pe_jsonshow(array('result'=>false, 'show'=>'已发货订单不能取消'));
			if (!$_p_order_closetext) pe_jsonshow(array('result'=>false, 'show'=>'请填写订单取消原因'));
	...
	case 'success':
		...
		if (in_array($info['order_payway'], array('alipay_db', 'cod'))) pe_jsonshow(array('result'=>false, 'show'=>'参数错误'));
	...
	case 'comment':
		...
		$info = $db->pe_select('order', array('order_id'=>$order_id, 'user_id'=>$_s_user_id));
		if (!$info['order_id']) pe_jsonshow(array('result'=>false, 'show'=>'参数错误'));
		if ($info['order_state'] != 'success') pe_jsonshow(array('result'=>false, 'show'=>'订单未未完成'));
	...
	default:
		...
		elseif (in_array($_g_state, array('wpay', 'wsend', 'wget'))) {
