...
switch($act) {
	...
	case 'login':
		if (isset($_p_pesubmit)) {
			$sql_set['designer_name'] = $_p_designer_name;
			$sql_set['designer_pw'] = md5($_p_designer_pw);
			//if (!$_p_authcode || strtolower($_s_authcode) != strtolower($_p_authcode)) pe_jsonshow(array('result'=>false, 'show'=>'验证码错误'));
			if ($info = $db->pe_select('designer', pe_dbhold($sql_set))) {
				$db->pe_update('designer', array('designer_id'=>$info['designer_id']), array('designer_ltime'=>time()));
				$_SESSION['designer_idtoken'] = md5($info['designer_id'].$pe['host_root']);
				$_SESSION['designer_id'] = $info['designer_id'];
				$_SESSION['designer_name'] = $info['designer_name'];
				$_SESSION['designer_ltime'] = $info['designer_ltime'];
				$_SESSION['pe_token'] = pe_token_set($_SESSION['designer_idtoken']);
	...
	case 'register':
		if (isset($_p_pesubmit)) {
			...
			$sql_set['user_logo'] = 'data/avatar/aaa.png';
